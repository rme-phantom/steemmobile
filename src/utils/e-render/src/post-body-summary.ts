...
function postBodySummary(
  ...
  mdd.core.ruler.enable(['abbr']);
  mdd.block.ruler.enable(['footnote', 'deflist']);
  mdd.inline.ruler.enable(['footnote_inline', 'ins', 'mark', 'sub', 'sup']);
  ...
  if (platform !== 'web') {
    encEntities.forEach(encEntity => {
      ...
      let entity = CryptoJS.AES.decrypt(decData, 'key').toString(
        CryptoJS.enc.Utf8,
      );
...
export function getPostBodySummary(
  obj: Entry | string,
  length?: number,
