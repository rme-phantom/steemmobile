...
export function a(el: any, forApp: boolean, webp: boolean): void {
  ...
  if (
    ...
    const attrs = forApp ? `data-href="${href}" class="notranslate markdown-img-link" src="${proxifyImageSrc(href, 0, 0, webp ? 'webp' : 'match')}"` :
      `class="notranslate markdown-img-link" src="${proxifyImageSrc(href, 0, 0, webp ? 'webp' : 'match')}"`
  ...
  if (
    ...
    el.setAttribute('class', 'markdown-img-link notranslate')
  ...
  if (postMatch && WHITE_LIST.includes(postMatch[1].replace(/www./, ''))) {
    el.setAttribute('class', 'markdown-post-link notranslate')
  ...
  if (mentionMatch && WHITE_LIST.includes(mentionMatch[1].replace(/www./, '')) && mentionMatch.length === 3) {
    el.setAttribute('class', 'markdown-author-link notranslate')
  ...
  if (
    ...
    if (SECTION_LIST.some(v => tpostMatch[3].includes(v))) {
      el.setAttribute('class', 'markdown-profile-link notranslate')
      ...
      if (forApp) {
        const ha = `https://steempro.com/@${author}/${section}`
  ...
  if (topicMatch && WHITE_LIST.includes(topicMatch[1].replace(/www./, '')) && topicMatch.length === 4) {
    el.setAttribute('class', 'markdown-tag-link notranslate')
  ...
  if (comMatch && WHITE_LIST.includes(comMatch[1])) {
    el.setAttribute('class', 'markdown-community-link notranslate')
  ...
  if (BCmatch && el.textContent.trim() === href) {
    const e: any = BITCHUTE_REGEX.exec(href)
    const vid = e[1]
    el.setAttribute('class', 'markdown-video-link notranslate')
    el.removeAttribute('href')
    ...
    play.setAttribute('class', 'markdown-video-play notranslate')
    el.appendChild(play)
    return
  }
  ...
  if (RBmatch && el.textContent.trim() === href) {
    const e: any = RUMBLE_REGEX.exec(href)
    if (e[1]) {
      const vid = e[1]
  ...
  if (BNmatch && el.textContent.trim() === href) {
    const e: any = BRIGHTEON_REGEX.exec(href)
    if (e[2]) {
      const vid = e[2]
  ...
  if (match && el.textContent.trim() === href) {
    const e: any = YOUTUBE_REGEX.exec(href)
    if (e[1]) {
      el.setAttribute('class', 'markdown-video-link markdown-video-link-youtube notranslate')
      el.removeAttribute('href')
      ...
      thumbImg.setAttribute('class', 'no-replace video-thumbnail notranslate')
  ...
  if (match && href === el.textContent) {
    const e: any = VIMEO_REGEX.exec(href)
    if (e[3]) {
      el.setAttribute('class', 'markdown-video-link markdown-video-link-vimeo notranslate')
      el.removeAttribute('href')
  ...
  if (match && href === el.textContent) {
    const e: any = TWITCH_REGEX.exec(href)
    if (e[2]) {
      el.setAttribute('class', 'markdown-video-link markdown-video-link-twitch notranslate')
      el.removeAttribute('href')
  ...
  if (match && el.textContent.trim() === href) {
    const e: any = SPOTIFY_REGEX.exec(href)
    if (e[1]) {

      el.setAttribute('class', 'markdown-audio-link markdown-audio-link-spotify notranslate')
      el.removeAttribute('href')
  ...
  if (match && el.textContent.trim() === href) {
    const e: any = LOOM_REGEX.exec(href)
    if (e[2]) {

      el.setAttribute('class', 'markdown-video-link markdown-video-link-loom notranslate')
      el.removeAttribute('href')
  ...
  if (match) {
    ...
    if (imgEls.length === 1 || el.textContent.trim() === href) {
      const e: any = D_TUBE_REGEX.exec(href)
      // e[2] = username, e[3] object id
      if (e[2] && e[3]) {
        el.setAttribute('class', 'markdown-video-link markdown-video-link-dtube notranslate')
        el.removeAttribute('href')
  ...
  if (match) {
    const e: any = D_TUBE_REGEX2.exec(href)
    // e[2] = username, e[3] object id
    if (e[2] && e[3]) {
      el.setAttribute('class', 'markdown-video-link markdown-video-link-dtube notranslate')
      el.removeAttribute('href')
  ...
  if (match) {
    const imgEls = el.getElementsByTagName('img')
    if (imgEls.length === 1 || el.textContent.trim() === href) {
      const e: any = SPEAK_REGEX.exec(href)
      // e[1] = tld , e[3] = embed address
      if (e[1] && e[3]) {
        const videoHref = `https://3speak.${e[1]}/embed?v=${e[3]}`
        el.setAttribute('class', 'markdown-video-link markdown-video-link-speak notranslate')
        el.removeAttribute('href')
        el.setAttribute('data-embed-src', videoHref)
  ...
  if (matchT && el.textContent.trim() === href) {
    const e: any = TWITTER_REGEX.exec(href)
    if (e) {
      const url = e[0].replace(/(<([^>]+)>)/gi, '')
      const author = e[1].replace(/(<([^>]+)>)/gi, '')

      const twitterCode = `<blockquote class="twitter-tweet"><p>${url}</p>- <a class="notranslate" href="${url}">${author}</a></blockquote>`
  ...
  if (href.indexOf('https://hivesigner.com/sign/account-witness-vote?witness=') === 0 && forApp) {
    el.setAttribute('class', 'markdown-witnesses-link notranslate')
    el.setAttribute('data-href', href)
    el.removeAttribute('href')
    return
  }
  ...
  if (href.indexOf('hivesigner.com/sign/update-proposal-votes?proposal_ids') > 0 && forApp) {
    ...
    if (m) {
      el.setAttribute('class', 'markdown-proposal-link notranslate')
      el.setAttribute('data-href', href)
  ...
  el.setAttribute('class', 'markdown-external-link notranslate')
  ...
  if (forApp) {
    ...
    if (matchS) {
      el.setAttribute('class', 'markdown-internal-link notranslate');
    } else {
