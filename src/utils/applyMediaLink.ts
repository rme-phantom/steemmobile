import { MediaInsertData } from "../components/basicComponents/ImageUploadButton";
...
interface Args {
    text: string;
    selection: Selection;
    setTextAndSelection: ({ selection, text }: {
...
export default async ({ text, selection, setTextAndSelection, items }: Args) => {
    ...
    items.forEach((item) => {
        ...
        switch (item.status) {
            case 'uploading': // means only filename is available
                if (!_placeholder) return;
                _insertFormatedString(item.text, _placeholder);
                break;

            case 'ready': // means url is ready but filename may be available
            ...
            case 'failed': // filename available but upload failed
                if (_placeholder && newText.includes(_placeholder)) {
