    ...
    const slices = chart.curves.map((c, i) => {
      ...
      if (absolute) {
        ...
        if (total === 0) {
          ...
          const percentage = (100 / total) * c.item[this.props.accessor];
        ...
        <G key={Math.random()}>
          ...
          {hasLegend ? (
            <Rect
              ...
              rx={0}
              ry={4}
              x={this.props.width / 2.8 - 10}
              y={
                -(this.props.height / 2.5) +
                ((this.props.height * 0.7) / this.props.data.length) * i +
                12 * 1
              }
            />
          ) : null}
          {hasLegend ? (
            <Text
              ...
              x={this.props.width / 2.7}
              y={
                -(this.props.height / 2.3) +
                ((this.props.height * 0.7) / this.props.data.length) * i +
                12 * 2
              }>
              {`${value} ${c.item.name}`}
