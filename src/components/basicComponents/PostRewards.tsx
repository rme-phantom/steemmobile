...
const PostRewards = (props: Props) => {
  const {comment} = props;
  const postRewardKey = `post-rewards-${comment.author}-${comment.permlink}`;
  const globalData = useAppSelector(state => state.steemGlobalReducer.value);

  const {data, isLoading, error} = useQuery({
    queryKey: [postRewardKey],
  ...
  const rewards = () => {
    if (data) {
      ...
      const authorPayout = data.payout - data.curator_payout_value;
      const curationPayout = data.curator_payout_value;
      const pendingPayout = data.payout;

      const totalPayout = data.total_payout_value;

      const maxPayout = data.max_accepted_payout;

      const payoutDate = data.cashout_time;

      const payoutLimitHit = totalPayout >= maxPayout;

      // assemble breakdown
      const sbdPrintRate = globalData.sbd_print_rate || 0;
      const percent_steem_dollars = data.percent_steem_dollars / 20000;
      ...
      // estimated pending payout breakdowns
      const _sbd = data.pending_payout_value * percent_steem_dollars;
      const pending_sp = (data.pending_payout_value - _sbd) / price_per_steem;
      const pending_sbd =
        _sbd * (globalData.sbd_print_rate / SBD_PRINT_RATE_MAX);
      ...
      return (
        <VStack>
          {pendingPayout > 0 && (
            <HStack items="center">
              <Text>
                {!!payoutDate ? 'Pending payout amount:' : `Past Payouts`}{' '}
              </Text>
          ...
          {!payoutDate && authorPayout > 0 && (
            <Text style={styles.itemIndent}>
              Author ${authorPayout.toFixed(3)}
            </Text>
          )}

          {!payoutDate && curationPayout > 0 && (
            <Text style={styles.itemIndent}>
              Curators ${curationPayout.toFixed(3)}
            </Text>
          )}
          {payoutLimitHit && <Text>{maxPayout}</Text>}

          {!!breakdownPayout && pendingPayout > 0 && !!payoutDate && (
            <VStack>
              <Text>Breakdown:</Text>
          ...
          {beneficiaries?.length > 0 && <Text>{beneficiaries}</Text>}
